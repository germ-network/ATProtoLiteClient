...
struct CustomDateFormatterLite: Sendable {

	/// A shared, singleton instance for global access.
	static let shared = CustomDateFormatterLite()
...
extension KeyedDecodingContainer {
	...
	/// Decodes a non-optional date string using the provided `CustomDateFormatterLite`.
	///
	...
	public func decodeDateLite(forKey key: Key) throws -> Date {
		let dateString = try self.decode(String.self, forKey: key)
		guard let date = CustomDateFormatterLite.shared.date(from: dateString) else {
	...
	public func decodeDateIfPresentLite(forKey key: Key) throws -> Date? {
		if let dateString = try self.decodeIfPresent(String.self, forKey: key) {
			guard let date = CustomDateFormatterLite.shared.date(from: dateString)
			else {
				throw DecodingError.dataCorruptedError(
					forKey: key, in: self,
					debugDescription: "Invalid date format: \(dateString)")
...
extension KeyedEncodingContainer {
	...
	public mutating func encodeDateLite(_ date: Date, forKey key: Key) throws {
		if let dateString = CustomDateFormatterLite.shared.string(from: date) {
	...
	///   - key: The key associated with the encoded date.
	/// - Throws: `EncodingError` if the date cannot be encoded.
	public mutating func encodeDateIfPresentLite(_ date: Date?, forKey key: Key) throws {
		if let date = date {
			if let dateString = CustomDateFormatterLite.shared.string(from: date) {
				try self.encode(dateString, forKey: key)
			} else {
