...
public protocol ATProtoErrorLite: Error {}
...
/// `ATRecordProtocolLite` conforms to `Codable`; all `Codable`-related features still
...
/// `private(set)` keyword:
/// ```swift
/// public struct UserProfile: ATRecordProtocolLite {
...
public protocol ATRecordProtocolLite: Sendable, Codable, Equatable, Hashable {
...
public protocol ATRecordConfigurationLite {

	/// An array of record lexicon structs created by Bluesky.
	///
	/// If `canUseBlueskyRecords` is set to `false`, these will not be used.
	var recordLexicons: [any ATRecordProtocolLite.Type] { get }

	/// Internal state to track the process of adding records to ``ATRecordConfigurationLite``.
	var initializationTask: Task<Void, Error>? { get }

	/// A method for setting the process of adding records to ``ATRecordTypeRegistryLite``.
	func checkInitialization() async throws
}

extension ATRecordConfigurationLite {
...
/// the registry. This is used as a map for ``UnknownTypeLite`` to find the most appropriate type
/// that the JSON object will fit into.
///
/// This registry is managed by an `actor`, ensuring concurrency-safe access to its internal
/// data. Because of this, most interactions with `ATRecordTypeRegistryLite` must be done
/// asynchronously using `await`.
///
/// When adding a record, you need to type `.self` at the end.
/// ```swift
/// Task {
///     _ = await ATRecordTypeRegistryLite.shared.register(types: [UserProfile.self])
/// }
/// ```
///
/// - Important: Make sure you don't add the same `struct` multiple times.
///
/// - Warning: All record types _must_ conform to ``ATRecordProtocolLite``. Failure to do so may
/// result in an error.
///
...
/// use ``ATRecordTypeRegistryLite/waitUntilRegistryIsRead()`` in the function. This prevents the
/// function from continuing until the registry is ready to be used.
///
/// - Note: ``ATRecordTypeRegistryLite/waitUntilRegistryIsRead()`` only works in functions that
/// are `async`.
public actor ATRecordTypeRegistryLite {

	/// The shared instance of `ATRecordTypeRegistryLite`.
	public static let shared = ATRecordTypeRegistryLite()

	/// A private dispatch queue for the registry.
	private let registryQueue = DispatchQueue(
		label: "com.cjrriley.ATProtoKit.ATRecordTypeRegistryLiteQueue")

	/// A private property of ``recordRegistry``.
	/// Global state protected by the registryQueue
	nonisolated(unsafe) private(set) static var _recordRegistry:
		[String: any ATRecordProtocolLite.Type] = [:]
	...
	/// lexicon (which is used as the "key"), and the ``ATRecordProtocolLite``-conforming `struct`s
	/// (which is used as the "value"). ``UnknownTypeLite`` will search for the key that matches with
	...
	public static var recordRegistry: [String: any ATRecordProtocolLite.Type] {
		return self.shared.registryQueue.sync {
			Self._recordRegistry
		}
	}

	/// Indicates whether any Bluesky-related `ATRecordProtocolLite`-conforming `struct`s have been
	/// added to ``recordRegistry``. Defaults to `false`.
	///
	/// - Warning: Don't touch this property; this should only be used for ``ATProtoKit``.
	/// Global state protected by the registryQueue
	nonisolated(unsafe) public private(set) static var areBlueskyRecordsRegistered = false
	...
	///
	/// - Parameter types: An array of ``ATRecordProtocolLite``-conforming `struct`s.
	public func register(types: [any ATRecordProtocolLite.Type]) async {
	...
	///
	/// - Parameter blueskyLexiconTypes: An array of ``ATRecordProtocolLite``-conforming `struct`s.
	public func register(blueskyLexiconTypes: [any ATRecordProtocolLite.Type]) async {
	...
	/// - Returns: A `struct` or `class`, which conforms to ``ATRecordProtocolLite``.
	///
	...
	public static func createInstance(ofType type: String, from decoder: Decoder) throws -> (
		any ATRecordProtocolLite
	)? {
	...
	/// - Returns: The ``ATRecordProtocolLite``-conforming type (if there's a match), or `nil`
	/// (if there isn't).
	public static func getRecordType(for type: String) -> (any ATRecordProtocolLite.Type)? {
		self.shared.registryQueue.sync {
			return ATRecordTypeRegistryLite.recordRegistry[type]
		}
	}
...
/// representation.
///
/// Within the ``ATRecordProtocolLite``-conforming `struct`, any properties that are of type
/// `unknown` as dictated by the lexicon the `struct`conforms to _must_ of this type. For example,
/// here's the basic structure for ``AppBskyLexiconLite/Notification/Notification``:
/// ```swift
...
///     public let record: UnknownTypeLite // Records will be stored here.
...
/// ```
///
/// As shown above, the `records` property is of type `UnknownTypeLite`. By adding this, any `struct`s
/// within the dictionary of ``ATRecordTypeRegistryLite/recordRegistry`` can be used to potentially
/// decode and encode the JSON object.
public enum UnknownTypeLite: Sendable, Codable {

	/// Represents a decoded ``ATRecordProtocolLite``-conforming `struct`.
	case record(any ATRecordProtocolLite)

	/// Represents an unknown type.
	///
	/// When this is used, the JSON object is converted to `[String: Any]` object. It's your
	/// responsibility to handle this properly.
	case unknown([String: CodableValueLite])

	/// Initializes `UnknownTypeLite` by attempting to decode a known record type or falling back
	/// to a raw dictionary.
	///
	...
	public init(from decoder: Decoder) throws {
		...
		if let typeKey = DynamicCodingKeys(stringValue: "$type"),
			let typeIdentifier = try container.decodeIfPresent(
				String.self, forKey: typeKey),
			let recordType = ATRecordTypeRegistryLite.recordRegistry[typeIdentifier]
		{
			let record = try recordType.init(from: decoder)
			self = .record(record)
		} else {
			do {
				let dictionary = try UnknownTypeLite.decodeNestedDictionary(
					container: container)
	...
	/// Attempts to retrieve a record of the specified type from an `UnknownTypeLite` instance.
	///
	...
	///
	/// - Parameter type: An ``ATRecordProtocolLite``-conforming type.
	/// - Returns: An instance of the specified record type if the `UnknownTypeLite` contains a record
	/// of that type.
	public func getRecord<Record: ATRecordProtocolLite>(ofType type: Record.Type) -> Record? {
	...
	/// Converts the `UnknownTypeLite` into a `[String: CodableValueLite]`.
	///
	...
	///
	/// - Returns: A `[String: CodableValueLite]` version. of the instance.
	///
	/// - Throws: An error if the conversion fails.
	public func asCodableValueLite() throws -> [String: CodableValueLite] {
		switch self {
		case .unknown(let dictionary):
			return dictionary
		case .record(let record):
			...
			guard let dictionary = jsonObject as? [String: Any] else {
				throw CodableValueLiteConversionError.invalidRecordConversion
			}
			return try dictionary.mapValues { try CodableValueLite.fromAny($0) }
		}
	}

	/// Represents possible errors during conversion.
	public enum CodableValueLiteConversionError: ATProtoErrorLite {
	...
	private static func decodeNestedDictionary(
		container: KeyedDecodingContainer<DynamicCodingKeys>
	) throws -> [String: CodableValueLite] {
		var dictionary = [String: CodableValueLite]()
		for key in container.allKeys {
			...
			{
				dictionary[key.stringValue] = .dictionary(
					try decodeNestedDictionary(container: nestedContainer))
			} else if let array = try? decodeArray(from: container, forKey: key) {
				dictionary[key.stringValue] = .array(array)
			} else {
				throw DecodingError.typeMismatch(
					CodableValueLite.self,
					DecodingError.Context(
						codingPath: container.codingPath,
						debugDescription:
							"Could not decode item at key \(key.stringValue)"
					))
			}
		}
		return dictionary
	...
	private static func decodeArray(
		...
		while !unkeyedContainer.isAtEnd {
			...
			} else if let nestedContainer = try? unkeyedContainer.nestedContainer(
				keyedBy: DynamicCodingKeys.self)
			{
				array.append(
					.dictionary(
						try decodeNestedDictionary(
							container: nestedContainer)))
			} else {
				throw DecodingError.dataCorruptedError(
					in: unkeyedContainer,
					debugDescription: "Could not decode array element.")
			}
		}
		return array
	}
	...
	public func encode(to encoder: Encoder) throws {
		...
		case .unknown(let unknownData):
			let jsonData = try JSONSerialization.data(
				withJSONObject: unknownData, options: [])
...
extension UnknownTypeLite: Equatable {
	public static func == (lhs: UnknownTypeLite, rhs: UnknownTypeLite) -> Bool {
		do {
			// Convert both sides to CodableValueLite dictionaries
			let lhsValue = try lhs.asCodableValueLite()
			let rhsValue = try rhs.asCodableValueLite()
...
extension UnknownTypeLite: Hashable {
	public func hash(into hasher: inout Hasher) {
		do {
			// Convert the UnknownTypeLite instance to a CodableValueLite
			let codableValueLite = try self.asCodableValueLite()
			// Hash the CodableValueLite representation
			codableValueLite.hash(into: &hasher)
		} catch {
			// If conversion fails, hash a fallback value
			hasher.combine("UnknownTypeLiteError")
		}
	}
}
...
public enum CodableValueLite: Codable, Sendable, Equatable, Hashable {
	...
	/// Stores an `Array` of `CodableValueLite` elements.
	case array([CodableValueLite])

	/// Stores a `Dictionary` with `String` keys and `CodableValueLite` values.
	case dictionary([String: CodableValueLite])
	...
	public init(from decoder: Decoder) throws {
		...
		if let value = try? container.decode(Bool.self) {
			...
			self = .string(value)
		} else if let value = try? container.decode([CodableValueLite].self) {
			self = .array(value)
		} else if let value = try? container.decode([String: CodableValueLite].self) {
	...
	/// Converts an `Any` type to a `CodableValueLite` type.
	///
	/// - Parameter value: The value to convert.
	/// - Returns: A `CodableValueLite` value.
	///
	/// - Throws: A `DecodingError` where the value was not supported.
	public static func fromAny(_ value: Any) throws -> CodableValueLite {
		switch value {
		...
		case let arrayValue as [Any]:
			let codableArray = try arrayValue.map { try CodableValueLite.fromAny($0) }
			return .array(codableArray)
		case let dictValue as [String: Any]:
			var codableDict = [String: CodableValueLite]()

			for (key, nestedValue) in dictValue {
				codableDict[key] = try CodableValueLite.fromAny(nestedValue)
			}
