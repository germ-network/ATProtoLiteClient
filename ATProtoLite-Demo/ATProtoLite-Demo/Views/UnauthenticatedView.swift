...
struct UnauthenticatedView: View {
	@AppStorage("unauthHandle") var username: String = "anna.germnetwork.com"
	...
	@State private var messagingDelegate: GermLexicon.MessagingDelegateRecord?
			...
			if let did {
				List {
					...
					Section("Profile") {
						...
						if let avatarBlob {
							if let uiImage = UIImage(data: avatarBlob) {
						...
						if let bannerBlob {
							if let uiImage = UIImage(data: bannerBlob) {
								Image(uiImage: uiImage)
									.resizable(
										resizingMode:
											.stretch
					...
					Section("Messaging delegate") {
						if let messagingDelegate {
							Text(
								"**Current key:** \(messagingDelegate.currentKey.base64EncodedString())"
							)
							Text(
	...
	private func loadAll() {
		...
		let newTask = Task {
			print("Loading DID...")
			do {
				did = try await ATProtoPublicAPI.getTypedDID(handle: username)
			} catch {
